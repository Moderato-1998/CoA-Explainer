...
class CoAExplainer:
    ...
    def _actions_to_edge_mask(self, graph_data, node_action_logits, temperature=None):
        ...
        if self.actor.training:
            temp = float(temperature) if temperature is not None else float(self.gumbel_temperature)
            temp = max(1e-3, temp)
            dist = torch.distributions.RelaxedBernoulli(temperature=temp, logits=edge_logits)
            ...
            if num_edges > 0 and edge_mask.sum() == 0:
                ...
                edge_scores[max_idx] = torch.maximum(edge_scores[max_idx], torch.tensor(0.51, device=edge_scores.device))
