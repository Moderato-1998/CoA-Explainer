...
class CoAExplainer_Base:
    ...
    def train_on_graph(self, graph_data, num_episodes_per_graph=1):
        ...
        # Get S_t: node embeddings from state_gnn on original graph
        # state_gnn is in train() mode; grads can flow if actor_input_node_embeddings is not detached
        _, actor_input_node_embeddings = self.state_gnn(
            graph_data.x.to(self.device),
            graph_data.edge_index.to(self.device),
            torch.zeros(graph_data.num_nodes,
                        dtype=torch.long, device=self.device)
        )
        # Pass non-detached embeddings to _get_all_agent_states for actor input
        actor_input_states = actor_input_node_embeddings
        num_nodes_in_graph = actor_input_states.size(0)
        ...
        for episode in range(num_episodes_per_graph):
            # Actor selects actions based on S_t (original graph embeddings)
            # Re-fetch embeddings for actor if state_gnn is updated within num_episodes_per_graph loop by update_agents()
            # For simplicity, assume state_gnn is fixed per call to train_on_graph, or use above actor_input_states
            # If update_agents() is called inside this loop, then actor_input_node_embeddings should be recomputed.
            # Current structure: update_agents() is called outside train_on_graph, so actor_input_states is fine.

            action_logits_per_node = self.select_actions(
                actor_input_states, add_noise=True)  # A_t
            edge_mask, _ = self._actions_to_edge_mask(
                graph_data, action_logits_per_node)
            ...
            if not hasattr(subgraph_data, 'num_edges'):  # Ensure num_edges is set
                subgraph_data.num_edges = subgraph_data.edge_index.size(1)

            reward, raw_fidelity, sparsity_score = self._calculate_reward(
                graph_data, subgraph_data, original_pred_logits, pred_target_class_idx
            )
            total_rewards_graph.append(reward)
            raw_fidelity_rewards_graph.append(raw_fidelity)
            sparsity_scores_graph.append(sparsity_score)

            # Prepare S_t and A_t for buffer (detached)
            state_for_buffer_unpadded = actor_input_node_embeddings.detach()
            actions_for_buffer_unpadded = action_logits_per_node.detach()

            current_state_padded, current_actions_padded = self._get_padded_global_states_actions(
                state_for_buffer_unpadded, actions_for_buffer_unpadded
            )

            # Prepare S_{t+1}: node embeddings from state_gnn on subgraph_data
            # subgraph_data.num_nodes and .x are same as original graph due to _get_subgraph implementation
            # Only edge_index changes.
            with torch.no_grad():  # S' embeddings should not have grads for target calculations
                _, next_node_embeddings_from_subgraph = self.state_gnn(
                    # Node features (same as original)
                    subgraph_data.x.to(self.device),
                    subgraph_data.edge_index.to(self.device),  # Subgraph edges
                    torch.zeros(subgraph_data.num_nodes, dtype=torch.long,
                                device=self.device)  # Batch for subgraph
                )
            next_state_for_buffer_unpadded = next_node_embeddings_from_subgraph  # Already detached

            # Dummy actions for padding S' (actions are not part of S')
            dummy_actions_for_s_prime_padding = torch.zeros(
                (next_state_for_buffer_unpadded.size(0), self.agent_action_dim), device=self.device)
            next_state_padded, _ = self._get_padded_global_states_actions(
                next_state_for_buffer_unpadded,
                dummy_actions_for_s_prime_padding
            )

            self.store_experience(current_state_padded, current_actions_padded,
                                  reward, next_state_padded, True, num_nodes_in_graph)
