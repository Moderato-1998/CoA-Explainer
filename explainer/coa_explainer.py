...
class CoAExplainer:
    def __init__(
        ...
        self.state_gnn = state_encode  # State GNN for actor input
        ...
        self.target_actor = Actor(self.agent_state_dim, self.agent_action_dim).to(
            device
        )
        ...
        self.critic = Critic(
            self.critic_global_state_dim, self.critic_global_action_dim
        ).to(device)
        ...
        # Relaxed Bernoulli/Gumbel-Sigmoid options: always used in training (ST), eval uses threshold
    ...
    def _actions_to_edge_mask(self, graph_data, node_action_logits, temperature=None):
        ...
        # src_importances = node_action_logits[edge_index[0]]  # (E, 1)
        # tgt_importances = node_action_logits[edge_index[1]]  # (E, 1)
        # src_logits = src_importances.squeeze(-1)  # (E,)
        # tgt_logits = tgt_importances.squeeze(-1)  # (E,)
        # edge_logits = src_logits * tgt_logits
        # src_probs = torch.sigmoid(src_logits)
        # tgt_probs = torch.sigmoid(tgt_logits)
        # edge_probs = src_probs * tgt_probs  # (E,)

        # Training-time: relaxed Bernoulli (Concrete) with straight-through estimator (default behavior)
        if self.actor.training:
            temp = (
                float(temperature)
                if temperature is not None
                else float(self.gumbel_temperature)
            )
            temp = max(1e-3, temp)
            dist = torch.distributions.RelaxedBernoulli(
                temperature=temp, logits=edge_logits
            )
            y_soft = dist.rsample()  # differentiable sample in (0,1)

            # Straight-through: hard mask forward, soft gradient backward
            y_hard = (y_soft > 0.5).float()
            y = y_hard.detach() - y_soft.detach() + y_soft
            edge_mask = y_hard.bool()
            edge_scores = y

            # Safety: ensure at least one edge kept to avoid degenerate empty graph early in training
            if num_edges > 0 and edge_mask.sum() == 0:
                max_idx = torch.argmax(edge_probs)
                edge_mask[max_idx] = True
                edge_scores = edge_scores.clone()
                edge_scores[max_idx] = torch.maximum(
                    edge_scores[max_idx], torch.tensor(0.51, device=edge_scores.device)
                )

            return edge_mask, edge_scores

        # Default behavior (previous): Bernoulli during training, threshold at eval
        if self.actor.training:
        ...
        # Return mask and edge probabilities (used by eval/explain for ranking)
        return edge_mask, edge_probs
    ...
    def _calculate_reward(
        ...
        try:
            with torch.no_grad():
                ...
                if masked_edge_index.numel() > 0:
                    ...
                    e_idx_sub, _ = tg_subgraph(
                        nodes_exp,
                        original_graph_data.edge_index.to(self.device),
                        relabel_nodes=True,
                    )
                    x_sub_nodes = original_graph_data.x.to(self.device)[nodes_exp]
                    b_sub_nodes = torch.zeros(
                        x_sub_nodes.size(0), dtype=torch.long, device=self.device
                    )
                    subgraph_pred_logits, _ = self.gnn_model_to_explain(
                        x_sub_nodes, e_idx_sub, b_sub_nodes
                    )
                else:
                    subgraph_pred_logits, _ = self.gnn_model_to_explain(
            ...
            raw_fidelity_term = (
                sub_log_probs[0, pred_target_class_idx]
                - full_log_probs[0, pred_target_class_idx]
            ).item()
        except Exception:
            raw_fidelity_term = -10.0
        ...
        M = (
            int(original_graph_data.num_edges)
            if hasattr(original_graph_data, "num_edges")
            else original_graph_data.edge_index.size(1)
        )
        K = (
            int(subgraph_data.num_edges)
            if hasattr(subgraph_data, "num_edges")
            else subgraph_data.edge_index.size(1)
        )
    ...
    def _get_subgraph(self, original_graph_data, edge_mask):
        ...
        if (
            original_graph_data.edge_index is None
            or original_graph_data.edge_index.size(1) == 0
        ):
            subgraph_data.edge_index = torch.empty(
                (2, 0), dtype=torch.long, device=self.device
            )
            if (
                hasattr(subgraph_data, "edge_attr")
                and subgraph_data.edge_attr is not None
            ):
                subgraph_data.edge_attr = torch.empty(
                    (0, subgraph_data.edge_attr.size(1)),
                    dtype=subgraph_data.edge_attr.dtype,
                    device=self.device,
                )
        ...
        if (
            hasattr(original_graph_data, "edge_attr")
            and original_graph_data.edge_attr is not None
        ):
            if subgraph_data.edge_index.size(1) > 0:  # if there are edges left
                ...
                subgraph_data.edge_attr = torch.empty(
                    (0, original_graph_data.edge_attr.size(1)),
    ...
    def store_experience(
        self, state, action, reward, next_state, done, num_nodes_for_experience
    ):
    ...
    def update_agents(self):
        ...
        for i in range(self.batch_size):
            ...
            action_padded = self._pad_tensor(action_unpadded, self.max_nodes, dim=0)
        ...
        actor_loss = -self.critic(states_batch, actor_actions_pred_batch).mean()
        ...
        if (
            self.state_gnn
        ):  # Also clip state_gnn if it's part of actor_params and being trained
    ...
    def train_on_graph(self, graph_data, num_episodes_per_graph=1):
        ...
        pred_target_class_idx = torch.argmax(original_pred_logits, dim=-1).item()
        ...
        for episode in range(num_episodes_per_graph):
            ...
            for step in range(max(1, int(self.steps_per_episode))):
                ...
                _, node_embeddings_t = self.state_gnn(
                    ...
                    torch.zeros(
                        current_graph.num_nodes, dtype=torch.long, device=self.device
                    ),
                )

                # 2) Actor selects actions per node
                action_logits_per_node = self.select_actions(
                    node_embeddings_t, add_noise=True
                )

                # 3) Build next subgraph by masking current_graph edges
                edge_mask_t, _ = self._actions_to_edge_mask(
                    current_graph,
                    action_logits_per_node,
                ...
                if next_graph.x is None:
                    next_graph.x = torch.empty(
                        (0, self.num_node_features), device=self.device
                    )
                if next_graph.edge_index is None:
                    next_graph.edge_index = torch.empty(
                        (2, 0), dtype=torch.long, device=self.device
                    )
                if not hasattr(next_graph, "batch") or next_graph.batch is None:
                    next_graph.batch = torch.zeros(
                        next_graph.x.size(0), dtype=torch.long, device=self.device
                    )
                ...
                state_t_padded, actions_t_padded = (
                    self._get_padded_global_states_actions(
                ...
                with torch.no_grad():
                    _, node_embeddings_tp1 = self.state_gnn(
                        ...
                        torch.zeros(
                            next_graph.num_nodes, dtype=torch.long, device=self.device
                        ),
                    )
                next_state_unpadded = node_embeddings_tp1
                dummy_actions_for_pad = torch.zeros(
                    (next_state_unpadded.size(0), self.agent_action_dim),
                    device=self.device,
                )
                next_state_padded, _ = self._get_padded_global_states_actions(
                    next_state_unpadded, dummy_actions_for_pad
                )

                # 7) done flag at final step or when graph becomes edgeless
                done_flag = (step == max(1, int(self.steps_per_episode)) - 1) or (
                    next_graph.num_edges == 0
                )
        ...
        avg_total_reward = np.mean(total_rewards_graph) if total_rewards_graph else 0
        avg_raw_fidelity = (
            np.mean(raw_fidelity_rewards_graph) if raw_fidelity_rewards_graph else 0
        )
        avg_sparsity_score = (
            np.mean(sparsity_scores_graph) if sparsity_scores_graph else 0
        )
    ...
    def evaluate_graph(self, graph_data):
        ...
        if graph_data.num_nodes == 0:
            empty_subgraph = Data(
                x=torch.empty((0, self.num_node_features), device=self.device),
        ...
        with torch.no_grad():
            ...
            original_pred_class = torch.argmax(original_pred_logits, dim=-1).item()
        ...
        edge_mask, edge_probs = self._actions_to_edge_mask(
            graph_data, action_logits_per_node, temperature=None
        )
        ...
        if (
            subgraph_data.num_edges == 0 and graph_data.num_edges > 0
        ):  # If subgraph is empty but original was not
            ...
            num_edges_to_keep = min(max(1, int(graph_data.num_edges * 0.1)), 10)
        ...
        if not hasattr(subgraph_data, "batch") or subgraph_data.batch is None:
            subgraph_data.batch = torch.zeros(
                subgraph_data.x.size(0), dtype=torch.long, device=self.device
            )
        ...
        if subgraph_data.num_nodes > 0:  # Can predict if nodes exist, even if no edges
            try:
                ...
                subgraph_pred_class = torch.argmax(subgraph_pred_logits, dim=-1).item()
    ...
    def explain(self, graph_data_to_explain):
        ...
        if graph_data_to_explain.num_nodes == 0:
            return graph_data_to_explain.clone(), torch.empty(
                0, dtype=torch.bool, device=self.device
            )
        if (
            graph_data_to_explain.edge_index is None
            or graph_data_to_explain.edge_index.size(1) == 0
        ):
            return graph_data_to_explain.clone(), torch.empty(
                0, dtype=torch.bool, device=self.device
            )
        ...
        with torch.no_grad():
            _, node_embeddings_for_explain = self.state_gnn(
                ...
                torch.zeros(
                    graph_data_to_explain.num_nodes,
                    dtype=torch.long,
                    device=self.device,
                ),
            )
        ...
        explanation_edge_mask, edge_probs = self._actions_to_edge_mask(
            graph_data_to_explain, node_action_logits
        )  # actor is in eval mode
        explanation_subgraph_data = self._get_subgraph(
            graph_data_to_explain, explanation_edge_mask
        )

        if (
            explanation_subgraph_data.num_edges == 0
            and graph_data_to_explain.num_edges > 0
        ):
