...
class SubgraphX(_BaseExplainer):
    ...
    def get_explanation_node(self, 
        ...
        if label is None:
            self.model.eval()
            pred, _ = self.model(x.to(device), edge_index.to(device), **forward_kwargs)
            label = int(pred.argmax(dim=1).item())
        else:
            label = int(label)

        # collect all the class index
        logits, _ = self.model(x.to(device), edge_index.to(device), **forward_kwargs)
    ...
    def get_explanation_graph(self, 
        ...
        if label is None:
            self.model.eval()
            pred, _ = self.model(x, edge_index, **forward_kwargs)
            label = int(pred.argmax(dim=1).item())

        # collect all the class index
        logits, _ = self.model(x, edge_index, **forward_kwargs)
        probs = F.softmax(logits, dim=-1)
        probs = probs.squeeze()
    ...
    def __parse_results(self, best_subgraph, edge_index):
        ...
        subgraph_nodes = torch.tensor([map[c] for c in best_subgraph.coalition], dtype=torch.long, device=device) if map is not None \
            else torch.tensor(best_subgraph.coalition, dtype=torch.long, device=device)

        # Create node mask:
        node_mask = torch.zeros(all_nodes.shape, dtype=torch.bool, device=device)
