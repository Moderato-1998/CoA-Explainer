...
class PGExplainer(_BaseExplainer):
    ...
    def __init__(self, model: nn.Module, emb_layer_name: str = None,
        ...
        # Infer in_channels based on the embedding layer used in user's GNN model
        if in_channels is None:
            base_out = None

            # Case 1: Linear layer
            if isinstance(self.emb_layer, torch.nn.Linear):
                base_out = self.emb_layer.out_features

            # Case 2: Layers that wrap an internal MLP with out_features (e.g., GINConv, custom GINWConv)
            elif hasattr(self.emb_layer, 'nn') and hasattr(self.emb_layer.nn, 'out_features'):
                base_out = self.emb_layer.nn.out_features

            # Case 3: MessagePassing layers exposing out_channels (e.g., GCNConv, GATConv, etc.)
            elif hasattr(self.emb_layer, 'out_channels'):
                base_out = getattr(self.emb_layer, 'out_channels')

            # If we still couldn't infer, raise a helpful error
            if base_out is None:
                fmt_string = (
                    'PGExplainer cannot infer in_channels from embedding layer of type {}. '
                    'Please provide in_channels explicitly.'
                )
                raise NotImplementedError(fmt_string.format(type(self.emb_layer)))

            # Special handling for attention heads (e.g., GATConv)
            # Effective output dim is out_channels * heads when concat=True
            if hasattr(self.emb_layer, 'heads'):
                heads = getattr(self.emb_layer, 'heads', 1)
                concat = getattr(self.emb_layer, 'concat', True)
                if concat:
                    base_out = base_out * heads

            in_channels = mult * base_out
