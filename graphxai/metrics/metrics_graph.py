...
def graph_exp_faith_graph(generated_exp: Explanation, data: Data, 
    ...
    #     pert_vec, _ = model(pert_x.to(device), EIDX)[generated_exp.node_idx]
    ...
    #     pert_vec, _ = model(pert_x, EIDX, **forward_kwargs)#[generated_exp.node_idx]
    #     pert_softmax = F.softmax(pert_vec, dim=-1)
    ...
    if generated_exp.edge_imp is not None:
        ...
        # Getting the softmax vector for the perturbed graph
        pert_vec, _ = model(X, edge_index.to(device), **forward_kwargs)#[generated_exp.node_idx]
        pert_softmax = F.softmax(pert_vec, dim=-1)        
