...
class Explanation:
    ...
    def visualize_graph(self, ax=None, show=False, agg_nodes=torch.mean, n_color='skyblue'):
        '''
        ...
        G = gxai_utils.to_networkx_conv(self.graph, to_undirected=False)
        ...
        if self.edge_imp is not None:
            ...
            edge_list = edge_heat.tolist()
            # If edge importance is binary (0s and 1s), use specific colors and widths.
            if all(v in [0, 1] for v in edge_list):
                draw_args['edge_color'] = ['red' if v == 1 else 'gray' for v in edge_list]
                draw_args['width'] = [3.0 if v == 1 else 1.0 for v in edge_list]
            else:
                # Otherwise, use the values for colormapping.
                draw_args['edge_color'] = edge_list
            draw_args['edge_cmap'] = plt.cm.coolwarm

        # Don't do anything for feature imp
        pos = nx.kamada_kawai_layout(G)
        nx.draw_networkx(G, pos, ax=ax, arrows=False, with_labels=False, **draw_args)
