...
class GraphDataset:
    def __init__(self, name, split_sizes=(0.7, 0.2, 0.1), seed=None, device=None, shuffle=True):
        ...
        if split_sizes[1] > 0:
            self.train_index, self.test_index = train_test_split(torch.arange(start=0, end=len(self.graphs)),
                                                                 test_size=split_sizes[1] + split_sizes[2], random_state=self.seed, shuffle=shuffle)
        else:
        ...
        self.num_features = self.graphs[0].x.size(1)
        self.num_classes = int(self.Y.max().item()) + 1
        self.max_nodes = max(g.x.size(0) for g in self.graphs)
        self.ave_nodes = sum(g.x.size(0) for g in self.graphs) / len(self.graphs)
        self.ave_edges = sum(g.edge_index.size(1) for g in self.graphs) / len(self.graphs)
