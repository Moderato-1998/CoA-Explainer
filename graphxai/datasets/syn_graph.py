...
class SynGen(NodeDataset):
    ...
    def __init__(self,
        ...
        if isinstance(shape, nx.Graph):
            ...
            if shape == 'house':
                ...
                self.insert_shape = diamond
            elif shape == 'circle':
                self.insert_shape = pentagon  # 6-member ring
            elif shape == 'random':
                self.insert_shape = star
            # if shape == 'random':
    ...
    def explanation_generator(self, node_idx):
        ...
        original_in_num_hop = set([self.G.nodes[n]['shape'] for n in khop_subgraph_nx(node_idx, 1, self.G) if self.G.nodes[n]['shape'] == self.G.nodes[node_idx]['shape']])
        ...
        edge_index = khop_info[1].transpose(1, 0)
        for i in range(len(edge_index)):
            u = int(edge_index[i, 0])
            v = int(edge_index[i, 1])
            if self.G.has_edge(v, u):
                ...
                edge_index[i, 0] = u
                edge_index[i, 1] = v
        original_edge_in_num_hop = set([self.G.edges[int(e[0]), int(e[1])]['shape']
                                       for e in edge_index if self.G.edges[int(e[0]), int(e[1])]['shape'] == self.G.nodes[node_idx]['shape']])
        # Get edge importance based on edges between any two nodes in motif
        # edge_imp_map =  {e:(self.G.edges[int(e[0]),int(e[1])]['shape'] in original_edge_in_num_hop) for e in edge_index}
        ...
        for i in range(khop_info[1].shape[1]):
            ...
            edge_imp[i] = self.G.edges[u, v]['shape'] in original_edge_in_num_hop
...
def generate_house():
    ...
    ran = [np.random.random() for _ in range(1000)]
    subgraph_size = [random.randint(8, 12) for _ in range(1000)]
    for num in range(1000):
        if ran[num] < 0.5:
            # subgraph_size = random.randint(15, 25)
            print(subgraph_size[num])
            G = BBG_PA(
                shape=house,
                num_subgraphs=1,
                inter_sg_connections=1,
                prob_connection=0,
                subgraph_size=subgraph_size[num],
                num_hops=1,
                seed=1234
            )
            y = 1
            gen_labels = bound_graph_label(G)
            ys = torch.tensor([gen_labels(i) for i in G.nodes], dtype=torch.long)
            yvals = ys.detach().clone()  # MUST COPY TO AVOID MAJOR BUGS

            gen_features, feature_imp_true = gaussian_lv_generator(
                G, yvals, seed=1234,
                n_features=10,
                class_sep=1.0,
                n_informative=4,
            ...
            # x = torch.stack([gen_features(i) for i in G.nodes]).float()
            # edge_index = to_undirected(torch.tensor(list(G.edges), dtype=torch.long).t().contiguous())
            edge_index = []
            ...
            original_edge_in_num_hop = set([G.edges[int(edge_index[0, e]), int(edge_index[1, e])]['shape']
                                           for e in range(edge_index.shape[1]) if G.edges[int(edge_index[0, e]), int(edge_index[1, e])]['shape'] == 1])

            edge_imp = torch.zeros(edge_index.shape[1], dtype=torch.double)
            sumk = 0
            for i in range(edge_index.shape[1]):
                u = int(edge_index[0, i])
                v = int(edge_index[1, i])

                edge_imp[i] = G.edges[u, v]['shape'] in original_edge_in_num_hop
                sumk += edge_imp[i]
            if sumk != 12:
                print("!!!")
            ground_truth_list.append(edge_imp)
        else:
            # subgraph_size = random.randint(15, 25)
            if ran[num] < 0.7:
                G = BBG_PA(
    ...
    out = [edge_index_list, feature_list, label_list, ground_truth_list]
...
def generate_diamond():
    ...
    ran = [np.random.random() for _ in range(1000)]
    subgraph_size = [random.randint(10, 15) for _ in range(1000)]
    for num in range(1000):
        if ran[num] < 0.5:
            # subgraph_size = random.randint(15, 25)
            print(subgraph_size[num])
            ...
            y = 0
            gen_labels = bound_graph_label(G)
            ys = torch.tensor([0 for i in G.nodes], dtype=torch.long)
            yvals = ys.detach().clone()  # MUST COPY TO AVOID MAJOR BUGS
            ...
            for edge in list(G.edges):
                edge_index.append([edge[0], edge[1]])
                edge_index.append([edge[1], edge[0]])
            edge_index = torch.tensor(list(G.edges), dtype=torch.long).transpose(1, 0)
            edge_index_list.append(edge_index)
            x = torch.stack([gen_features(i) for i in G.nodes]).float()
            feature_list.append(x)
            label_list.append(y)
...
def generate_cycle():
    ...
    ran = [np.random.random() for _ in range(1000)]
    subgraph_size = [random.randint(8, 11) for _ in range(1000)]
    for num in range(1000):
        if ran[num] < 0.6:
            # subgraph_size = random.randint(15, 25)
            print(subgraph_size[num])
...
def generate_grid():
    ...
    ran = [np.random.random() for _ in range(1000)]
    subgraph_size = [random.randint(7, 10) for _ in range(1000)]
    for num in range(1000):
        if ran[num] < 0.6:
            # subgraph_size = random.randint(15, 25)
            print(subgraph_size[num])
    ...
    print(label_list)
    process_path = os.path.join(os.path.dirname(__file__), 'BA-Grid.npy')
    out = [edge_index_list, feature_list, label_list, ground_truth_list]
    np.save(process_path, arr=np.array(out, dtype=object), allow_pickle=True)
