...
def ba_around_shape(shape: nx.Graph, add_size: int, show_subgraphs: bool = False):
    ...
    for i in range(add_size):
        ...
        shape.edges[connect_node, new_node]['shape'] = 0
    if show_subgraphs:
...
def build_bound_graph(
    ...
    for i in range(num_subgraphs):
        ...
        nx.set_edge_attributes(current_shape, shape_number, 'shape')
    ...
    for i in range(len(subgraphs)):
        G.add_edges_from(subgraphs[i].edges(data=True))
...
#    print(G.edges[:]['shape'])
    ...
    for i, j in tqdm.tqdm(iter_edges):
        ...
        while np.sum(running_mask) > 0:
            ...
            G.add_edge(rand_edge[0], rand_edge[1], shape=0)
